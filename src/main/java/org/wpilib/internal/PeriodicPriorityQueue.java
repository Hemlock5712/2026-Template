...
public class PeriodicPriorityQueue {
  ...
  public boolean runCallbacks(int notifier) {
    ...
    if (m_useTiming) {
      ...
      // Free-running: the caller (log replay) sets the pace, so never sleep. Loop start comes
      // from the schedule rather than the clock, so the queue still advances exactly one period
      // per cycle and every periodic callback keeps its relative rate.
